...
def synthesize_adaptive_policy(
    ...
    *,
    ...
    rank: dict[Belief, int] = {belief: 0 for belief in reachable if terminal(belief)}
        ...
        for belief in reachable:
            ...
            for action in system.actions:
                ...
                if not successors or any(successor not in rank for successor in successors):
                    continue
                candidate = 1 + max(rank[successor] for successor in successors)
                proposal = (candidate, action)
                if best is None or proposal < best:
                    best = proposal
            if best is not None and (belief not in rank or best[0] < rank[belief]):
                rank[belief] = best[0]
                choice[belief] = best[1]
                changed = True
